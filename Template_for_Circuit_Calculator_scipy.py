...
|___________________|
| THIS TEMPLATE	 USES
| scipy MODULE FOR SOLVING
| THE EIGENVALUE PROBLEM
| THIS METHOD IS MORE EFFICIENT
| THAN numpy BUT CALCULATES ONLY
| N_States_to_Calc LOWEST
| energy states
|___________________ 
"""
...
# Define numbeor of states that will be calculated during
# diagonalization of the Hamiltonian.
# Such calculation is much faster than
# diagonalization used in the "numpy" module.
N_States_to_Calc = 6
...
def plottingFunc(t, EigVals):
    ...
    # number of bands (eigenenstates) to plot
    N_bands_to_plot =  N_States_to_Calc
    plt.plot(
...
def showProgressMsg(t_current, EigVals_at_current_t):
    ...
    N_bands_to_show = N_States_to_Calc
    SQM.print_text_in_line( "t = " + str(round(_t,4)), color = 35 )
    print( "\nE_n/K:" )
...
def showInitialMsg(EasySQM):
    ...
    print( np.hstack([EasySQM.SQM.ModelCalc.Matrices.Z_cyc,
                      EasySQM.SQM.ModelCalc.Matrices.M_osc.round(3)]) )
    print( "_____oOOOo_____" )
