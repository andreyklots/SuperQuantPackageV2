...
def eigsort(A, METHOD = None, u_initial = None):
    ...
    if (METHOD=="H")or(METHOD=="h")or(METHOD=="hermit")or(METHOD=="Hermit"):
        ...
        E, u = LA.eigh( np.matrix(_A) )
        return [np.matrix(E).T, np.matrix(u)]
    # Below are methods using sparse module. The syntax is 
    # METHOD = "[methodtype][N_eigs]" with [methodtype]--being a method used
    # and [N_eigs]--being a number of eigenstates to calculate
    if METHOD[0:7] == "sparseH":
        #Hermitize matrix
        _A = 0.5*(A+A.H)
        # Number of states to get
        NofEigs = int( METHOD[ 7:len(METHOD) ] )
        E, u = spLA.eigsh(_A, k = NofEigs, which='SA')
        #sort eigenvalues by their real part
        idx = np.argsort(E.real)
        E = E[idx]
        u = u[:,idx]                          
        return [np.matrix(E).T, np.matrix(u)]
    if METHOD[0:6] == "sparse":
        # Number of states to get
...
KRONTYPE = "numpy"
def MDkron(A,B):
    if KRONTYPE=="numpy" or KRONTYPE=="np":
        return np.kron(A,B)
...
def print_text_in_line(Text, color=None):
    if color != None:
        # if color integer is defined, make colored text
        _Text = '\x1b['+str(color)+'m' + Text + '\x1b[0m'
    else:
        _Text = Text
    try:
        exec( 'print(_Text, end = "")' ) # print inline for Python3
                       # if fail then try Python2 code below
    except:
        try:
            exec( 'print _Text,' ) 
        except:
    ...
    def __calc_Flux_Charge_Biases(self, Phi, Q,    G, H, m_cyc, U, Inv_sqrt_C):
        ...
        if np.size(G) > 0:
            ...
            try:
                ...
                print(  "\n\n[ERROR] Could not obtain F=(H\\G)Phi."\
                      + " Likely G has duplicate rows"\
                      + " (duplicate inductors).\n\n")
                F = "[ERROR] Could not obtain F=(H\\G)Phi."\
                  + " Likely G has duplicate rows (duplicate inductors)."
        else:
    ...
    def __obtainHamiltonian(self, N_cyc, N_osc, m_cyc, \
        ...
        for N_current in N:
            ...
            for I_current in I:
                ...
                if current_I_index == current_N_index:
                    ...
                    current_N_tot = MDkron(current_N_tot, N_current)
                else:
                    # everywhere else put I-operator of corresponding dimen
                    # sions
                    current_N_tot = MDkron(current_N_tot, I_current)
        ...
        for m in range(N_cyc, N_cyc+N_osc):
            ...
            Hamilt_osc = Hamilt_osc \
                       + MDkron( omega_eigenvalues[m], (_N[m] + 0.5*_I) )
        ...
        for m in range(0,N_Jos):
            ...
            for n in range(0,N_cyc+N_osc):
                _row_product=MDkron(_row_product,V[m][n])
...
class EasyModel:
    ...
    # nding frequencies
    def summarizeCoord(self, roundN=None):
        self.printMessage( "" )
        ...
        self.printMessage("Transformation II matrix Mat: "
                          ...
                          + str( self.SQM.ModelCalc.getTransform2Matrix()\
                                 if roundN == None else
                      self.SQM.ModelCalc.getTransform2Matrix().round(roundN) )
                         )
        self.printMessage( "---------------" )
